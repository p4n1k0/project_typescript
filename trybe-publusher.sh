...
if [[ ! -d $SCRIPTS_BASE ]]; then
    echo -e "${CYAN}Obtendo base de configurações de limpeza${NO_COLOR}"
    ...
    echo -e "${CYAN}Atualizando base de configurações de limpeza${NO_COLOR}"
    CURR_PATH=$(pwd)
...
if ! git fetch origin --quiet ; then # não tem acesso ou porque não existe ou porque não tem autorização
    ...
    if [[ $REMOTE_DIFF_STAT ]]; then
        echo -e "${YELLOW}O repositório remoto possui algumas diferenças em relação ao local: "
        echo
