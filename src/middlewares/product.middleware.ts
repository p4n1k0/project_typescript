...
export const nameValidate = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (name.length <= 2) return res.status(422).json({ message: '"name" length must be at least 3 characters long' });
...
export const amountValidate = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (amount.length <= 2) return res.status(422).json({ message: '"amount" length must be at least 3 characters long' });
